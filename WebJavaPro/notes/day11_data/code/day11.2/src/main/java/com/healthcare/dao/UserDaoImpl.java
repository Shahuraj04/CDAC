...
public class UserDaoImpl implements UserDao {
	...
	public String loginUser(String email, String password) {
		...
		Transaction tx = session.beginTransaction();
		User user = null;
		String jpql = "select u from User u where email=:em and password=:pswd";
		try {
			user = session.createQuery(jpql, User.class).setParameter("em", email).setParameter("pswd", password)
					.getSingleResult();
			if (user.getRole().equals(UserRole.ROLE_PATIENT)) {
				return "Patient: " + user.toString();
			} else if (user.getRole().equals(UserRole.ROLE_DOCTOR)) {
