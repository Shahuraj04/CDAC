...
Simple eg : Refer to RaceCondition Demo

Here when we write 
- counter++
It looks like a like a single atomic operation (i.e single statement)
 but it’s actually 3 steps at the CPU level:

1. Read the value of counter from memory into a CPU register

2. Increment the register value by 1

3. Write the register value back to counter
...
Both increment -> each gets 11

Both write back 11

The correct result 12 is lost ,so  final count is less than expected.

Real world example  : Joint Bank Account : shared resource
...
How to apply synchronization(i.e thread safety) in java ?
Every Java object , is implicitly associated with a lock.
Any thread has to acquire the lock , while calling either synchronized method of synchronized block.
...
3. Can single thread acquire multip2. When does thrd need to acquire the lock (=enter the monitor)?-- if its invoking either synchronized methods or code from synchronized blocks
le locks -- YES
eg . Suppose thread t1 is calling A's synchronized method - show
class A {
