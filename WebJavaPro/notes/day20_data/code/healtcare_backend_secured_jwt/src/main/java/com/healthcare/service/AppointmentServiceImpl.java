...
//		return appointmentRepository.getPatientUpcomingAppointmentsByUserId2(userId,Status.SCHEDULED,LocalDateTime.now());
	...
	@Override
	public List<CompleteAppointmentResp> listAllAppointments() {

		return appointmentRepository.findAll()
				.stream()
				.map(entity -> mapper.map(entity, CompleteAppointmentResp.class))
				.toList();
	}
	@Override
	public List<AppointmentDTO> listUpcomingDoctorAppointments(Long doctorId) {
		// by patient id
		return appointmentRepository.getDoctorUpcomingAppointmentsByDoctorId(doctorId, Status.SCHEDULED);
		// by user id
//		return appointmentRepository.getPatientUpcomingAppointmentsByUserId2(userId,Status.SCHEDULED,LocalDateTime.now());
