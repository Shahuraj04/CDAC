...
i/p - user id.
	 */
	@PreAuthorize("hasRole('DOCTOR')") //method level access control - finer 
	//approach compared to url based access control. 
	@GetMapping("/{doctorId}/appointments/upcoming")	
	@Operation(description = "List doctor's upcoming appointments")
	public  ResponseEntity<?> listUpcomingAppointments(
			@PathVariable Long doctorId ,@AuthenticationPrincipal JwtDTO dto)
	{
