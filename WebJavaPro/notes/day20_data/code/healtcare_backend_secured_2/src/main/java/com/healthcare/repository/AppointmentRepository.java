...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
	...
	// custom query - to get list of truly upcoming appointments by user id (more
		...
		// custom query - to get list of upcoming appointments by doctor id
		@Query("""
				select new com.healthcare.dto.AppointmentDTO(a.id,a.appointmentDateTime,a.myPatient.userDetails.firstName,a.myPatient.userDetails.lastName) from Appointment a where a.myDoctor.id=:did and a.status=:sts order by a.appointmentDateTime asc
				""")
				List<AppointmentDTO> getDoctorUpcomingAppointmentsByDoctorId(@Param("did") Long doctorId, @Param("sts") Status scheduled);
}
