...
public class DoctorDaoImpl implements DoctorDao {
	...
	public DoctorDaoImpl() throws SQLException {
		...
		pst4 = cn.prepareStatement("select * from doctors where email=?");
		pst5 = cn.prepareStatement("select * from doctors where email=? and password=?");
	...
	public String signUp(Doctor newDoctor) throws SQLException {
		...
		pst4.setString(1, newDoctor.getEmail());
	...
	@Override
	public Doctor docSignIn(String email, String password) throws SQLException {
		pst5.setString(1,email);
		pst5.setString(2, password);
		try(ResultSet rs = pst5.executeQuery()){
			if(rs.next()){
				return new Doctor(rs.getString(2), rs.getString(3),rs.getString(4),rs.getString(5),rs.getDate(6));
			}
		}
